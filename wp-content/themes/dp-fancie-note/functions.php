...
//Version
define ('DP_OPTION_SPT_VERSION', '1.0.7.6');
//Base theme name
...
function dp_hook_template_redirect(){
	if (is_single()){
		$lm_date = dp_get_last_modified_date();
		header(sprintf("Last-Modified: %s", $lm_date));
	}
}
add_action( "template_redirect", "dp_hook_template_redirect" );
...
add_filter('the_content','dp_insert_widget_before_first_h_tag');
...
function dp_load_css_scripts() {
	...
	if ($options['decoration_type'] === 'bootstrap'){
		...
		wp_enqueue_script('bootstrap','https://maxcdn.bootstrapcdn.com/bootstrap/3.3.5/js/bootstrap.min.js',array('jquery'),DP_OPTION_SPT_VERSION,true);
	}
	// wow.js
	if ($IS_MOBILE_DP) {
		$mb_sufix = '_mb';
