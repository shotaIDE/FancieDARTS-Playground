	...
	<dl>
		...
		標準化設定 : 
			...
			<div class="pd15px-top pd25px-btm " style="position:relative;">
				...
				　→ <a href="https://digipress.info/manual/html-decoration/" target="_blank">HTML装飾リファレンス</a><br />
				　→ <a href="https://digipress.info/manual/icon-font-map/" target="_blank">アイコン一覧</a><br />
			...
			<div class="pd15px-top pd25px-btm formblock" style="position:relative;">
				<div>
					<input name="disable_canonical" id="disable_canonical" type="checkbox" value="check" <?php if($options['disable_canonical']) echo "checked"; ?> /><label for="disable_canonical">カノニカルタグ(rel="canonical")の自動出力を無効にする</label>
				</div>
				<div class="slide-title icon-info-circled mg12px-top"><?php _e('Note...', 'DigiPress'); ?></div>
				<div class="slide-content">
				※プラグインなどによるカノニカルタグの出力を優先する場合など、DigiPressによるカノニカルタグの自動出力を停止したい場合にこのオプションを有効にします。
				</div>
			</div>
			...
			<div class="pd15px-top pd15px-btm ">
				<div>
					<label><input name="disable_sns_share_count" type="checkbox" value="true" <?php if($options['disable_sns_share_count']) echo "checked"; ?> />SNSアイコン(ボタン)のみを表示し、シェア数は表示しない</label>
				</div>
				<div class="slide-title icon-info-circled mg12px-top"><?php _e('Note...', 'DigiPress'); ?></div>
				<div class="slide-content">
				※アーカイブページに表示するSNSシェア数やシングルページのSNSシェアボタンに、各SNSのシェア数は表示せず、SNSボタンのみを表示する場合にチェックします。<br />
				※このオプションを有効にすると、シェア数を取得するための Javascriptモジュールが読み込まれなくなり、シェア数の取得のために実行される各APIとの通信がなくなることで、ページ表示速度が向上します。
				</div>
			</div>

			<div class="pd15px-top pd15px-btm ">
				<div>
					<label><input name="disable_dns_prefetch" type="checkbox" value="true" <?php if (isset($options['disable_dns_prefetch']) && !empty($options['disable_dns_prefetch'])) echo "checked"; ?> />DNS プリフェッチ用の link タグを出力しない</label>
				</div>
				<div class="slide-title icon-info-circled mg12px-top"><?php _e('Note...', 'DigiPress'); ?></div>
				<div class="slide-content">
				※DNS プリフェッチとは、head セクション内に 専用の link タグを挿入し、外部ドメインの名前解決を事前に行い、表示速度を向上させる仕組みです。<br />
				※WordPress 4.6 以降から、ajax.googleapis.com などの DNS プリフェッチ用の link タグが自動で挿入されるようになり、この機能を無効にして DNS プリフェッチ用の link タグを出力しない場合はこのオプションを指定してください。
				</div>
			</div>
...
<?php submit_button(__(' Save ', 'DigiPress'), 'primary', 'dp_save'); ?>
</div>
</div>
<!--
========================================
...
<div class="mg10px-top mg20px-btm"><input class="button close_all" type="button" name="close_all" value=" <?php _e('Close All', 'DigiPress'); ?>" /></div><?php

// AMP Settings?>
<h3 class="dp_h3 icon-menu">AMP(Accelerated Mobile Pages)専用設定</h3>
<div class="dp_box">
<div class="pd30px">
	<div class="mg30px-btm">
		<div class="mg15px-btm"><label for="use_amp"><input name="use_amp" id="use_amp" type="checkbox" value="check" <?php if(isset($options['use_amp']) && !empty($options['use_amp'])) echo "checked"; ?> />AMPを有効化</label></div>
		<div class="mg20px-l mg15px-btm"><label for="always_redirect_to_amp"><input name="always_redirect_to_amp" id="always_redirect_to_amp" type="checkbox" value="check" <?php if(isset($options['always_redirect_to_amp']) && !empty($options['always_redirect_to_amp'])) echo "checked"; ?> />サイト内リンクは常時AMPページにリダイレクトする</label></div>
		<div class="mg20px-l"><label for="disable_amp_archive"><input name="disable_amp_archive" id="disable_amp_archive" type="checkbox" value="check" <?php if(isset($options['disable_amp_archive']) && !empty($options['disable_amp_archive'])) echo "checked"; ?> />シングルページのみAMPを有効にする</label></div>

		<div class="slide-title icon-info-circled mg12px-top"><?php _e('Note...', 'DigiPress'); ?></div>
		<div class="slide-content">
		※スマートフォンでのWebページのレンダリングをAMPの仕様に従って表示することで、<span class="red">通常のモバイルテーマより軽量化し、高速化</span>します。<br />　<a href="https://www.ampproject.org/ja/learn/about-how/" target="_blank">AMPの詳細や仕組みについてはこちら</a><br />
		※Googleのモバイル検索結果のみにAMPページを反映(識別)させるだけでなく、アクセス後に<span class="red">サイト内のページ遷移もすべてAMPページで表示</span>する場合は、「<span class="red">サイト内リンクは常時AMPページにリダイレクトする</span>」にチェックをしてください。<br />
		※<span class="red">任意の記事や固定ページでAMPを無効化</span>する必要がある場合は、対象記事の編集画面の<span class="red">「投稿オプション」にて、"このページではAMPを無効にする" にチェック</span>をして保存してください。<br />
		※AMPを有効にした場合、主に以下の制限があります。<br />
		<p class="label">AMP有効時の制限事項</p>
		<ul class="mg15px-l">
		<li>・<span class="red">オリジナルCSSはAMP専用に分けて追加</span>できますが、<span class="red">テーマのCSSと合わせて50KB以下</span>である必要があります。</li>
		<li>・AMP対応のページを表示するには、WordPressの<span class="red">パーマリンク設定にて「基本」(既定)以外のパーマリンクにしておく</span>必要があります。</li>
		<li>・AMPが有効になるのは、トップページ、既知のアーカイブページ(カテゴリー、タグ、日付、寄稿者、検索結果)、記事ページ、固定ページ、組み込みの「お知らせ」用カスタム投稿タイプが対象です。</li>
		<li>・テーマ以外の操作(プラグインなど)で組み込んだ、独自のカスタム投稿タイプのページや、その他自前のテンプレートで表示しているページでもAMPモードで表示するには、「AMPを有効にするカスタム投稿タイプ」オプションにて対象のカスタム投稿タイプのスラッグを指定してください。</li>
		<li>・トップページのヘッダーエリアのスライダーは、<span class="red">エフェクトは水平方向へのスライド表示のみ</span>となり、トランジション時間は反映されません。</li>
		<li>・アーカイブページの記事一覧には、<span class="red">SNSシェア数やページビュー数は表示されません</span>。</li>
		<li>・シングルページの記事メタエリアに表示される各SNSのシェアボタンは、<span class="red">フッターバーの「シェア」をタップしてポップアップで表示</span>されます。</li>
		<li>・<span class="red">アーカイブページの表示形式と「DP-アーカイブ専用記事一覧」ウィジェットは「ノーマル」形式のみ</span>となり、他の表示スタイルは反映されません。</li>
		<li>・<span class="red">「DP-タブウィジェット」、「DP-Facebookページプラグイン」など、Javascriptの処理を必要とするようなウィジェットはAMPページでは使用しない</span>でください。</li>
		<li>・サイト内検索ウィジェットは利用できません(※フッターバーの検索アイコンからポップアップでサイト内検索フォームを表示可能)。</li>
		<li>・「ビジュアル設定」にて指定している<span class="red">カテゴリーカラーは反映されません</span>。</li>
		<li>・HTMLフォーム(form)は非SSL通信(http)環境では利用できません(formタグが削除されます)。</li>
		<li>・Javascriptは利用できません。</li>
		<li>・&lt;!--nextpage--&gt;タグによるシングルページの分割は無効になり、1ページで表示されます。</li>
		</li>
		<li>・コメントが許可されている投稿ページにてコメントするには、非AMPページでのみ可能となります。</li>
		<li>・トラックバックは表示されません。</li>
		<li>・すべての<span class="red">プラグインで指定されているJavascriptとCSSは出力されなく</span>なります。</li>
		<li>・記事やテキストウィジェット内に指定している<span class="red">scriptタグやstyle属性によるインラインのJavascriptとCSSはすべて削除</span>されます。</li>
		<li>・記事内にimgタグで表示している<span class="red">画像はamp-imgタグに置換され、表示幅が250ピクセル以上の画像はブロック要素として左右への回り込みができません</span>。</li>
		<li>・テーマに組み込みのHTML装飾セレクタ(CSS)は、必要最低限のスタイルが読み込まれ、<span class="red">ボタンカラー(btn以外)、ラベルカラー(label以外)、マーカー、ボックスカラー(box以外)、40ピクセル以上のフォントサイズ(ft8px〜ft35px以外)、マージン(mg系)、パディング(pd系)、セリフ体(serif)、アニメーション(animate-*)のCSSの装飾は無効</span>になります。</li>
		<li>・WordPressのメディア追加機能のギャラリー(ショートコード)表示は、指定カラム数に関係なく3カラム固定で表示されます。</li>
		<li>・組み込みの「寄稿者一覧テンプレート」はAMPに対応していません。このテンプレートを指定した固定ページでは、投稿オプションからAMPを無効にしてください。</li>
		</ul>
		※<span class="red">AMP対応ページは、URLの最後に「/amp」を付けて表示</span>できます。<br />
		※AMPページのエラー確認などの<span class="red">検証を行うには、ブラウザ(Chrome)のディベロッパーコンツールを有効にし、URLの最後に「#development=1」を付けてAMPページを更新</span>することでコンソールで確認できます。<br />
		※AMPを有効にしている状態で、表示しているページの<span class="red">AMP化を一時的に無効にするには、URLの最後に「#nonamp=1」を付けてページを更新</span>してください。<br />
		※AMPを有効にしている状態で、<span class="red ft13px b">後からパーマリンク設定を変更したり、AMPページが404エラーで表示できなくなった場合</span>は、この<span class="red ft13px b">「詳細設定」のいずれかの「設定を保存」ボタンでそのまま保存し直し</span>てください。
		</div>
	</div>
	<div class="mg30px-btm">
		<h4 class="dp_set_title2 icon-triangle-right">スライドサイドメニュー設定</h4>
		<div class="mg20px-l">
			<label class="mg20px-r"><input name="sidemenu_pos_amp" type="radio" value="left" <?php if(!isset($options['sidemenu_pos_amp']) || $options['sidemenu_pos_amp'] === 'left') echo "checked"; ?> />左からスライド表示</label>
			<label><input name="sidemenu_pos_amp" type="radio" value="right" <?php if(isset($options['sidemenu_pos_amp']) && $options['sidemenu_pos_amp'] === 'right') echo "checked"; ?> />右からスライド表示</label>
		</div>
	</div>
	<div class="mg30px-btm">
		<h4 class="dp_set_title2 icon-triangle-right">非AMPページナビゲーション設定</h4>
		<div class="mg20px-l">
			<div class="mg15px-btm">
				<label>非AMPページへのリンクテキスト : </label><input id="nav_text_nonamp" name="nav_text_nonamp" type="text" placeholder="通常ページはこちら" value="<?php if (isset($options['nav_text_nonamp']) && !empty($options['nav_text_nonamp'])) {echo $options['nav_text_nonamp'];}else{echo $def_control['nav_text_nonamp'];} ?>" size="32" />
			</div>
			<div class="slide-title icon-info-circled mg12px-top"><?php _e('Note...', 'DigiPress'); ?></div>
			<div class="slide-content">
	...
	<div class="mg30px-btm">
		<h4 class="dp_set_title2 icon-triangle-right">アクセス解析(Google アナリティクス)設定</h4>
		<div class="mg20px-l">
			<div class="mg15px-btm">
				<label>Google アナリティクストラッキングID : </label><input id="ga_tracking_id" name="ga_tracking_id" type="text" placeholder="UA-XXXXX-Y" value="<?php if (isset($options['ga_tracking_id'])) echo $options['ga_tracking_id']; ?>" size="24" />
			</div>
			<div>
				<label for="no_track_admin_amp"><input name="no_track_admin_amp" id="no_track_admin_amp" type="checkbox" value="check" <?php if(isset($options['no_track_admin_amp']) && !empty($options['no_track_admin_amp'])) echo "checked"; ?> />管理者自身(ログイン中)はカウントしない</label>
			</div>
			<div class="slide-title icon-info-circled mg12px-top"><?php _e('Note...', 'DigiPress'); ?></div>
			<div class="slide-content">
			※AMPではscriptタグが禁止されているため、一般的なアクセス解析コードは利用できません。このオプションでは、「アクセス解析コード設定」に指定している<a href="https://analytics.google.com/" target="_blank">Googleアナリティクス</a>と<span class="red">同じトラッキングID(UA-XXXXX-Y)のみを指定</span>することで、テーマが<span class="red">自動でAMP専用の解析コードを生成</span>してbodyセクションに埋め込み、<span class="red">AMPページと非AMPページのアクセスを統合して解析</span>することができます。
			</div>
		</div>
	</div>
	<div class="mg30px-btm">
		<h4 class="dp_set_title2 icon-triangle-right">AMPを有効にするカスタム投稿タイプ</h4>
		<div class="mg20px-l">
			<div class="mg15px-btm">
				<label>カスタム投稿タイプのスラッグ : </label><input id="enable_amp_cpt" name="enable_amp_cpt" type="text" placeholder="music,movie,actor" value="<?php if (isset($options['enable_amp_cpt'])) echo $options['enable_amp_cpt']; ?>" size="38" />
			</div>
			<div class="slide-title icon-info-circled mg12px-top"><?php _e('Note...', 'DigiPress'); ?></div>
			<div class="slide-content">
			※プラグインなどで独自に追加されているカスタム投稿タイプのうち、<span class="red">AMPの表示対象とするカスタム投稿タイプがある場合は、その投稿タイプのスラッグを指定</span>します。<br />
			※AMPを有効にする対象のカスタム投稿タイプが<span class="red">複数ある場合は、スラッグを半角カンマ「,」で区切って指定</span>してください。<br />　例: music,movie,actor
			</div>
		</div>
	</div>
	<div class="mg30px-btm">
		<h4 class="dp_set_title2 icon-triangle-right">&lt;head&gt;～&lt;/head&gt;内のユーザー定義設定</h4>
		<div class="mg20px-l">
			<textarea name="custom_head_content_amp" id="custom_head_content_amp" cols="100%" rows="10"><?php if(isset($options['custom_head_content_amp']) && !empty($options['custom_head_content_amp'])) echo($options['custom_head_content_amp']); ?></textarea>
			<div class="slide-title icon-info-circled"><?php _e('Note...', 'DigiPress'); ?></div>
			<div class="slide-content">
			※headセクション内に任意のコードを挿入したい場合は、このオプションに記述してください。<span class="red">ただし、AMPでは、scriptタグや指定箇所以外でのCSSの記述は禁止されているため、<a href="https://www.ampproject.org/ja/docs/reference/spec" target="_blank">AMPの仕様</a>に則ったタグやコーディングの範囲</span>でカスタマイズしてください。
			</div>
		</div>
	</div>
	<div class="mg30px-btm">
		<h4 class="dp_set_title2 icon-triangle-right">&lt;body&gt;タグ直後のユーザー定義設定</h4>
		<div class="mg20px-l">
			<textarea name="custom_under_body_content_amp" id="custom_under_body_content_amp" cols="100%" rows="10"><?php if(isset($options['custom_under_body_content_amp']) && !empty($options['custom_under_body_content_amp'])) echo($options['custom_under_body_content_amp']); ?></textarea>
			<div class="slide-title icon-info-circled"><?php _e('Note...', 'DigiPress'); ?></div>
			<div class="slide-content">
			※Googleタグマネージャーの解析用コードやGoogle Analytics以外のAMPに対応したトラッキングサービスのコードなど、bodyタグの先頭に設置することが推奨、あるいは決められているサービスで発行されたコードがある場合は、このオプションに記述してください。<br />
			※AMPの<span class="red">「アクセス解析設定」にてトラッキングIDを指定している場合</span>は、<span class="red b ft13px">「&lt;head&gt;～&lt;/head&gt;内のユーザー定義設定」に "amp-analytics-0.1.js" のscriptタグの記述は不要</span>です。
			</div>
		</div>
	</div>
	<div class="mg30px-btm">
		<h4 class="dp_set_title2 icon-triangle-right">オリジナルスタイルシート設定</h4>
		<div class="mg20px-l">
			<textarea name="original_css_amp" id="original_css_amp" cols="100%" rows="10"><?php if(isset($options['original_css_amp']) && !empty($options['original_css_amp'])) echo($options['original_css_amp']); ?></textarea>
			<div class="slide-title icon-info-circled"><?php _e('Note...', 'DigiPress'); ?></div>
			<div class="slide-content">
			※<span class="red">AMPでは、CSSの総サイズが50KB以下</span>である必要があります。テーマに組み込みたいCSSがある場合は、<span class="red">テーマのCSSと合わせて50KBを超えない範囲</span>となるように注意してください。<br />
			※AMPページで<span class="red">CSSが50KBを超えてないか確認するには、ブラウザ(Chrome)のディベロッパーコンツールを有効にし、URLの最後に「#development=1」を付けてAMPページを更新</span>してコンソールを確認してください。CSSが50KBを超えていた場合、コンソールに「<span class="red">The author stylesheet specified in tag 'style' is too long - we saw (現在のCSSサイズ値) bytes whereas the limit is 50000 bytes.</span>」というエラーが表示されます。
			</div>
		</div>
	</div>
	<div class="mg30px-btm">
		<h4 class="dp_set_title2 icon-triangle-right">コメントエリア設定</h4>
		<div class="mg20px-l">
			<div class="mg15px-btm">
				<label>コメントエリアタイトル : </label><input id="comments_main_title_amp" name="comments_main_title_amp" type="text" placeholder="コメント一覧" value="<?php if (isset($options['comments_main_title_amp'])) echo $options['comments_main_title_amp']; ?>" size="48" />
			</div>
			<div class="slide-title icon-info-circled mg12px-top"><?php _e('Note...', 'DigiPress'); ?></div>
			<div class="slide-content">
			※AMPでは、制約上WordPressのコメントフォームを表示できないため、投稿済みのコメント一覧のみが表示されます。<br />
			※投稿済みのコメントに返信する場合、または新規にコメントする場合は、非AMPページに遷移します。<br />
			※トラックバックは表示されません。
			</div>
		</div>
	</div>
<div><?php submit_button(__(' Save ', 'DigiPress'), 'primary', 'dp_save', false); ?></div>
</div>
</div>
	...
	<dl>
		...
		&lt;head&gt;～&lt;/head&gt;内の&lt;meta&gt;タグに関する設定 :
			...
			<dd>
				...
				<div class="formblock mg15px-top mg20px-btm">
					<label><input name="disable_meta_keywords" id="disable_meta_keywords" type="checkbox" value="check" <?php if(isset($options['disable_meta_keywords']) && !empty($options['disable_meta_keywords'])) echo "checked"; ?> /> メタキーワード(meta name="keywords") を出力しない</label>
				</div>
			...
			<dd>
				...
				<div id="global_menu_right_div1" class="mg15px-l box-c">
					...
					<div class="mg20px-top">
						...
						<h3 class="dp_set_title2 icon-triangle-right">YouTube URL :</h3>
							<div class="dp_indent1">
							<div>
							<input type="text" name="global_menu_youtube_url" size="64" value="<?php echo($options['global_menu_youtube_url']); ?>" /><br />※サイト専用またはあなたのYouTubeアカウントのURL
							</div>
							</div>
						</div>
				...
				<div id="home_bottom_content_dd" class="box">
					...
					<h4 class="dp_set_title2 pd12px-btm">各記事の表示形式 :</h4>
						<div class="clearfix mg20px-l mg20px-btm">
								...
								<div id="top_listing_post_style1" class="bg-white mg15px-l mg15px-btm pd15px">
									...
									<div class="mg15px-l">
										...
										<div class="mg15px-l">
											...
											<div class="mg10px-btm">
												<input name="tweets_number_after_title_top" id="tweets_number_after_title_top"  type="checkbox" value="check" <?php if($options['tweets_number_after_title_top']) echo "checked"; ?> />
												<label for="tweets_number_after_title_top">Twitterのツイート数を表示(*)</label>
											</div>
							...
							※DigiPressの既定のNo Image用画像を変更する場合は、<span class="red">テーマフォルダの "/img/post_thumbnail/"</span>内の画像を自由に置き換えてください。<br />
						※ツイート数を取得して表示するには、<a href="https://jsoon.digitiminimi.com/" class="icon-new-tab" target="_blank">count.jsoon</a> というサービスにて、事前にサイトを登録しておく必要があります。
							</div>
						</div>
								...
								<div id="archive_listing_post_style1" class="bg-white mg15px-l mg15px-btm pd15px">
									...
									<div class="mg15px-l">
										...
										<div class="mg15px-l">
											...
											<div class="mg10px-btm">
											<input name="tweets_number_after_title_archive" id="tweets_number_after_title_archive"  type="checkbox" value="check" <?php if($options['tweets_number_after_title_archive']) echo "checked"; ?> />
											<label for="tweets_number_after_title_archive">Twitterのツイート数を表示(*)</label>
											</div>
...
<input name="show_pocket_button" id="show_pocket_button" type="checkbox" value="check" <?php if($options['show_pocket_button']) echo "checked"; ?> />
			...
			※「LINEで送る」ボタンは、モバイルテーマでのみ表示されます。<br />
			※表示スタイルを「オリジナル」にした場合は、Twitter, Facebook, Pocket, Pinterest, はてなブックマーク, Evernote, Feedly のシェアボタンのみが対象となります。
			</div>
			</dd>
