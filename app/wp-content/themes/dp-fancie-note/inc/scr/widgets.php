...
function dp_register_widget_area() {
	...
	register_sidebar(array(
			...
			'after_title'	=> '</span></h1>'));
	// Top page container for AMP
	register_sidebar(array(
			'name'		=> __('Container top area(AMP)', 'DigiPress'),
			'id'		=> 'widget-top-container-amp',
			'description'	=> __('Between under header area and main content.', 'DigiPress'),
			'before_widget'	=> '<div id="%1$s" class="widget-box %2$s">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h3 class="inside-title"><span>',
			'after_title'	=> '</span></h3>'));
	// Under the post title space for AMP
	register_sidebar(array(
			'name'		=> __('Under the post title(AMP)', 'DigiPress'),
			'id'		=> 'under-post-title-amp',
			'before_widget'	=> '<div id="%1$s" class="widget-box %2$s">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h3 class="inside-title"><span>',
			'after_title'	=> '</span></h3>'));
	...
	// Middle post
	register_sidebar(array(
			'name'		=> __('Middle of the post(AMP)', 'DigiPress'),
			'id'		=> 'widget-post-middle-amp',
			'before_widget'	=> '<div id="%1$s" class="widget-box %2$s">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h3 class="inside-title"><span>',
			'after_title'	=> '</span></h3>'));

	// Last post content for AMP
	register_sidebar(array(
			'name'		=> __('Bottom of the post(AMP)', 'DigiPress'),
			'id'		=> 'bottom-of-post-amp',
			'before_widget'	=> '<div id="%1$s" class="widget-box %2$s">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h3 class="inside-title"><span>',
			'after_title'	=> '</span></h3>'));

	// Container footer widget for AMP
	register_sidebar(array(
			'name'		=> __('Container bottom(AMP)', 'DigiPress'),
			'id'		=> 'widget-bottom-container-amp',
			'description'	=> __('After main content area for mobile.', 'DigiPress'),
			'before_widget'	=> '<div id="%1$s" class="widget-box %2$s">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h3 class="inside-title"><span>',
	...
	// Site footer widget for AMP
	register_sidebar(array(
			'name'		=> __('Footer Widget(AMP)', 'DigiPress'),
			'id'		=> 'footer-amp',
			'description'	=> __('Footer widget for mobile.', 'DigiPress'),
			'before_widget'	=> '<div id="%1$s" class="widget-box %2$s">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h2 class="inside-title"><span>',
			'after_title'	=> '</span></h2>'));
}
...
function DP_GET_POSTS_BY_QUERY($args = array(
	...
	foreach( $posts as $post ) : setup_postdata($post);
		...
		$post_title = '<h4 class="excerpt_title_wid"><a href="'.$post_url.'" rel="bookmark" class="item-link">'.$post_title.'</a></h4>';
		...
		// Thumbnail 
		if ($thumbnail) {
			$arg = array("width"=>$width, "height"=>$height, "size"=>"dp-widget-thumb");
			$thumb_code = '<div class="widget-post-thumb thumb">'.DP_Post_Thumbnail::get_post_thumbnail($arg).'</div>';
		}
		...
		if ($post_type === 'post') {
			...
			if ((bool)$hatebuNumber || (bool)$tweetsNumber || (bool)$likesNumber || $voted_count) {
				$sns_share_code = '<div class="loop-share-num ct-shares" data-url="'.$post_url.'">'.$sns_share_code.$comment_code.$voted_code.'</div>';
			}
			// ************* SNS sahre number *****************
		}

		// Combine
		$item_list_code .= '<li class="clearfix'.$rank_label_class.'">'.$thumb_code.'<div class="excerpt_div clearfix">'.$date_code.$post_title.$views_code.'</div>'.$sns_share_code.$ranking_code.$js_code.'</li>';
...
<span style="font-size:11px;color:red;"><?php _e('*Note : This option is available at only one widget.','DigiPress'); ?></span></p><?php
	}
	...
	function widget($args, $instance) {
		...
		if ($instance['rss']) {
			if ($instance['to_feedly']) {
				$rss = '<li><a href="https://feedly.com/i/subscription/feed/'.urlencode(get_feed_link()).'" target="_blank" title="Follow on feedly"><span class="r-wrap"><i class="icon-feedly"></i></span></a></li>';
			} else {
				$rss = '<li><a href="'
...
class DP_Tab_Widget extends WP_Widget {
	...
	function form($instance) {
		...
		$first_open_tab_code = '<div style="margin-bottom:20px;"><div class="b mg5px-btm"><label for="'.$first_open_tab_id.'">'.__('First open tab', 'DigiPress').':</label></div><div class="mg15px-l"><select name="'.$first_open_tab_name.'" id="'.$first_open_tab_id.'">'.$first_open_tab_code.'</select></div></div>';
		...
		$form_code .= '<p><label for="'.$numberid.'">'.__('Number to display','DigiPress').':</label>
			 <input type="number" min=1 name="'.$numbername.'" id="'.$numberid.'" value="'.$number.'" style="width:60px;" /></p>';
	...
	function widget($args, $instance) {
		...
		if ($first_open_tab === 'most_viewed' && $wd_most_viewed) {
			...
			$first_tab = 2;
		} else if ($first_open_tab === 'recent_comments' && $wd_recentComment) {
			$flag_first_tab = false;
			$first_tab = 3;
			...
			$flag_first_tab = false;
			$first_tab = 6;
		} else if ($first_open_tab === 'tags' && $wd_tagCloud) {
			$flag_first_tab = false;
			$first_tab = 7;
		...
		// Display code
		$widget_code = $before_widget.'<div class="dp_tab_widget">';

		// Tab list
		$widget_code .= '<ul class="dp_tab_widget_ul clearfix">';
		if ($wd_newPost) {
			if ($flag_first_tab) {
				$flag_first_tab = false;
			...
			if ($instance['title_newPost']) {
				$widget_code .= '<li id="tab_newPost" class="'.$tab_state_class.'">'.$title_newPost.'</li>';
			} else {
				$widget_code .= '<li id="tab_newPost" class="'.$tab_state_class.'">Recent Posts</li>';
			}
		}
		if ($wd_most_viewed) {
			if ($flag_first_tab) {
			...
			if ($instance['title_most_viewed']) {
				$widget_code .= '<li id="tab_most_viewed" class="'.$tab_state_class.'">'.$title_most_viewed.'</li>';
			} else {
				$widget_code .= '<li id="tab_most_viewed" class="'.$tab_state_class.'">Popular Posts</li>';
			}
		}
		if ($wd_recentComment) {
			if ($flag_first_tab) {
			...
			if ($instance['title_recentComment']) {
				$widget_code .= '<li id="tab_recentComment" class="'.$tab_state_class.'">'.$title_recentComment.'</li>';
			} else {
				$widget_code .= '<li id="tab_recentComment" class="'.$tab_state_class.'">Recent Comment</li>';
			}
		}
		if ($wd_most_commented) {
			...
			if ($instance['title_most_commented']) {
				$widget_code .= '<li id="tab_most_commented" class="'.$tab_state_class.'">'.$title_most_commented.'</li>';
			} else {
				$widget_code .= '<li id="tab_most_commented" class="'.$tab_state_class.'">Most Commented</li>';
			}
		}
		if ($wd_random) {
			if ($flag_first_tab) {
			...
			if ($instance['title_random']) {
				$widget_code .= '<li id="tab_random" class="'.$tab_state_class.'">'.$title_random.'</li>';
			} else {
				$widget_code .= '<li id="tab_random" class="'.$tab_state_class.'">Pickup</li>';
			}
		}
		if ($wd_category) {
			if ($flag_first_tab) {
			...
			if ($instance['title_category']) {
				$widget_code .= '<li id="tab_category" class="'.$tab_state_class.'">'.$title_category.'</li>';
			} else {
				$widget_code .= '<li id="tab_category" class="'.$tab_state_class.'">Category</li>';
			}
		}
		if ($wd_tagCloud) {
			if ($flag_first_tab) {
			...
			if ($instance['title_tagCloud']) {
				$widget_code .= '<li id="tab_tagCloud" class="'.$tab_state_class.'">'.$title_tagCloud.'</li>';
			} else {
				$widget_code .= '<li id="tab_tagCloud" class="'.$tab_state_class.'">Tag Cloud</li>';
			}
		}
		if ($wd_archive) {
			if ($flag_first_tab) {
			...
			if ($instance['title_archive']) {
				$widget_code .= '<li id="tab_archive" class="'.$tab_state_class.'">'.$title_archive.'</li>';
			} else {
				$widget_code .= '<li id="tab_archive" class="'.$tab_state_class.'">Archive</li>';
			}
		}
		$widget_code .= '</ul>';

		// Show Tab content
		$widget_code .= '<div class="dp_tab_contents">';
		// Recent Posts
		if ($wd_newPost) {
			if ($first_tab === 1) {
				$widget_code .= '<div id="tab_newPost_content" class="first_tab">';
			} else {
				$widget_code .= '<div id="tab_newPost_content">';
			}
		...
		if ($wd_most_viewed) {
			if ($first_tab === 2) {
				$widget_code .= '<div id="tab_most_viewed_content" class="first_tab">';
			} else {
				$widget_code .= '<div id="tab_most_viewed_content">';
			}
		...
		if ($wd_recentComment) {
			if ($first_tab === 3) {
				$widget_code .= '<div id="tab_recentComment_content" class="first_tab">';
			} else {
				$widget_code .= '<div id="tab_recentComment_content">';
			}
		...
		if ($wd_most_commented) {
			if ($first_tab === 4) {
				$widget_code .= '<div id="tab_most_commented_content" class="first_tab">';
			} else {
				$widget_code .= '<div id="tab_most_commented_content">';
			}
		...
		if ($wd_random) {
			if ($first_tab === 5) {
				$widget_code .= '<div id="tab_random_content" class="first_tab">';
			} else {
				$widget_code .= '<div id="tab_random_content">';
			}
		...
		if ($wd_category) {
			...
			if ($first_tab === 6) {
				$widget_code .= '<div id="tab_category_content" class="tab_category_content first_tab'.$cat_has_count.'">';
			} else {
				$widget_code .= '<div id="tab_category_content" class="tab_category_content'.$cat_has_count.'">';
			}
		...
		if ($wd_tagCloud) {
			if ($first_tab === 7) {
				$widget_code .= '<div id="tab_tagCloud_content" class="first_tab">';
			} else {
				$widget_code .= '<div id="tab_tagCloud_content">';
			}
		...
		if ($wd_archive) {
			if ($first_tab === 8) {
				$widget_code .= '<div id="tab_archive_content" class="first_tab">';
			} else {
				$widget_code .= '<div id="tab_archive_content">';
			}
