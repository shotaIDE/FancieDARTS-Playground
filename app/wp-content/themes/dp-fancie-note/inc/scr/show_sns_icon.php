...
function dp_show_sns_buttons_original($position = 'top', $echo = true) {
	...
	// *** row1
	if ($options['show_facebook_button']) {
		$code1 = '<div class="sitem bg-likes ct-fb"><a href="https://www.facebook.com/sharer/sharer.php?u='.$url.'&t='.$title.'" target="_blank" rel="nofollow"><i class="share-icon icon-facebook"></i>'.$share_num_prefix.$share_num_suffix.'</a></div>';
	}
	if ($options['show_twitter_button']) {
		$code1 .= '<div class="sitem bg-tweets ct-tw"><a href="https://twitter.com/intent/tweet?original_referer='.$url.'&url='.$url.'&text='.$title.'" target="_blank" rel="nofollow"><i class="share-icon icon-twitter"></i>'.$share_num_prefix.$share_num_suffix.'</a></div>';
	}
	if ($options['show_hatena_button'] ) {
		$code1 .= '<div class="sitem bg-hatebu ct-hb"><a href="http://b.hatena.ne.jp/add?mode=confirm&url='.$url.'&title='.$title.'" target="_blank" rel="nofollow"><i class="share-icon icon-hatebu"></i>'.$share_num_prefix.$share_num_suffix.'</a></div>';
	}
	if ($options['show_pinterest_button']) {
		// Eyecatch ( for pinterest)
		$image_url = '';
		...
		$desc = urlencode(strip_tags(get_the_excerpt()));
		$code1 .= '<div class="sitem bg-pinterest ct-pr"><a href="//www.pinterest.com/pin/create/button/?url='.$url.$image_url.'&description='.$desc.'" rel="nofollow" target="_blank"><i class="share-icon icon-pinterest"></i>'.$share_num_prefix.$share_num_suffix.'</a></div>';
	}
	...
	if ($options['show_pocket_button']) {
		...
		$code2 = '<div class="sitem bg-pocket ct-pk"><a href="https://getpocket.com/edit?url='.$url.'" target="_blank"><i class="share-icon icon-pocket"></i>'.$share_num_prefix.$share_num.$share_num_suffix.'</a></div>';
	}
	if ($options['show_evernote_button']) {
		if ( !(isset($options['disable_sns_share_count']) && !empty($options['disable_sns_share_count'])) ){
			$share_num = '<span class="icon-attach"></span>';
		}
		$code2 .= '<div class="sitem bg-evernote"><a href="https://www.evernote.com/noteit.action?url='.$url.'&title='.$title.'" target="_blank"><i class="share-icon icon-evernote"></i>'.$share_num_prefix.$share_num.$share_num_suffix.'</a></div>';
	}
	if ($options['show_feedly_button']) {
		if ( !(isset($options['disable_sns_share_count']) && !empty($options['disable_sns_share_count'])) ){
		...
		$code2 .= '<div class="sitem bg-feedly"><a href="https://feedly.com/i/subscription/feed/'.$rss_url.'" target="_blank" rel="nofollow"><i class="share-icon icon-feedly"></i>'.$share_num_prefix.$share_num.$share_num_suffix.'</a></div>';
	}
	if ($IS_MOBILE_DP) {
		if ($options['show_line_button']) {
			if ( !(isset($options['disable_sns_share_count']) && !empty($options['disable_sns_share_count'])) ){
				$share_num = '<span class="icon-email"></span>';
			} else {
				$share_num = '';
			}
			$code2 .= '<div class="sitem bg-line"><a href="line://msg/text/' . $title . '%0D%0A' . $url . '"><i class="share-icon icon-line"></i>'.$share_num_prefix.$share_num.$share_num_suffix.'</a></div>';
		}
	}
	if (!empty($code1) || !empty($code2)) {
		$html_code = '<div class="loop-share-num ct-shares'.$no_share_num_flag.'" data-url="'.get_permalink().'">'.$code1.$code2.'</div>';
	}
