...
function dp_post_list_get_elements($args) {
	global $post, $IS_MOBILE_DP, $IS_AMP_DP;
	/**
	 * Init params
	 */
	$meta_key 			= '';
	...
	$width = 800;
	$height = 800;
	$arg_thumb = array('width' => $width, 'height' => $height, "size"=>"dp-archive-thumb", "if_img_tag"=> true);
	// Slider js
	$js_slider = '';
	...
	if (!$IS_AMP_DP) {
		...
		// Whole share code
		$sns_share_code = ($hatebu_num || $tweets_num || $likes_num || !empty($sns_insert_content)) ? '<div class="loop-share-num ct-shares" data-url="'.get_permalink().'">'.$sns_share_code.$arr_meta['comments'].$sns_insert_content.'</div>' : '';
	}
	...
	//Post excerpt
	if ((int)$excerpt_length !== 0 && !$IS_AMP_DP) {
	...
	// Thumbail
	if (empty($videoID)) {
		if ($layout !== 'slider' &&  ($sl_img1 || $sl_img2 || $sl_img3) && !$IS_AMP_DP) {
		...
		switch ($video_service) {
			case 'Vimeo':
				if ( WP_Filesystem() ) {
					...
					if (!$vimeo_hash) {
						...
						$size = dp_get_image_size($vimeo_hash[0]['thumbnail_large']);
						if ($IS_AMP_DP) {
							if (is_array($size)) {
								$thumbnail_code = '<amp-img src="'.$vimeo_hash[0]['thumbnail_large'].'" width="'.$size[0].'" height="'.$size[1].'" alt="Thubmnail" layout="responsive" class="wp-post-image"><amp-img>';
							}
						} else {
							if (is_array($size)) {
								$thumbnail_code = '<img src="'.$vimeo_hash[0]['thumbnail_large'].'" width="'.$size[0].'" height="'.$size[1].'" alt="Thubmnail" class="wp-post-image" />';
							}
			...
			case 'YouTube':
			default:
				$size = dp_get_image_size('https://img.youtube.com/vi/'.$videoID.'/0.jpg');
				if ($IS_AMP_DP) {
					if (is_array($size)) {
						$thumbnail_code = '<amp-img src="//img.youtube.com/vi/'.$videoID.'/0.jpg" width="'.$size[0].'" height="'.$size[1].'" layout="responsive" alt="Thubmnail" class="wp-post-image"><amp-img>';
					}
				} else {
					$thumbnail_code = '<img src="//img.youtube.com/vi/'.$videoID.'/0.jpg" width="'.$size[0].'" height="'.$size[1].'" class="wp-post-image" />';
					$embed_code 	= '<div class="fit-vids"><iframe class="emb" src="//www.youtube.com/embed/'.$videoID.'/?wmode=transparent&hd=1&autohide=1&rel=0" frameborder="0" allowfullscreen></iframe></div>';
				}
				break;
		}
	}
...
function dp_show_post_list_for_archive_normal($args = array(), $arr_post_ele = array()) {
	...
	$more_code = '<div class="more-link"><a href="'.$post_url.'"><span class="r-wrap">'.$read_more_str.'</span></a></div>';

	$js_code = !empty($js_slider) ? '<script>'.$js_slider.'</script>' : '';
	...
	// title
	$title_line = '<div class="title-line">'.$date_code.'<h1 class="entry-title loop-title '.$layout.$has_date_class.'"><a href="'.$post_url.'" rel="bookmark">'.$post_title.'</a></h1></div>';
	...
	if (!empty($embed_code)) {
		...
		$eyecatch_code = '<div class="loop-post-thumb '.$layout.'"><a href="'.$post_url.'" rel="bookmark" class="thumb-link r-wrap"><span>'.$thumbnail_code.'</span>'.$media_icon_code.'</a>'.$post_label.'</div>';
	}
	// Whole
	$loop_code .= 
'<article class="loop-article'.$col_class.$cat_class.$wow_article_css.'"><div class="loop-col one">'.$eyecatch_code.'</div><div class="loop-col two"><div class="loop-article-content">'.$title_line.$meta_code.$desc.$sns_share_code.$more_code.'</div></div>'.$js_code.'</article>';
...
function dp_show_post_list_for_archive_normal_mb($args = array(), $arr_post_ele = array()) {
	...
	// AMP
	if ($IS_AMP_DP) {
		$sns_share_code = !empty($arr_meta['comments']) ? '<div class="loop-share-num">'.$arr_meta['comments'].'</div>' : '';
	} else {
		$js_code = !empty($js_slider) ? '<script>'.$js_slider.'</script>' : '';
	}
	...
	// title
	$title_line = '<h1 class="entry-title loop-title '.$layout.$has_date_class.'"><a href="'.$post_url.'" rel="bookmark">'.$post_title.'</a></h1>';
	...
	if (!empty($embed_code)) {
		...
		$eyecatch_code = '<div class="loop-post-thumb '.$layout.'"><a href="'.$post_url.'" rel="bookmark" class="thumb-link">'.$thumbnail_code.'</a>'.$post_label.'</div>';
	}
	// Whole
	$loop_code .= 
'<article class="loop-article'.$cat_class.$wow_article_css.'"><div class="loop-col one">'.$eyecatch_code.'</div><div class="loop-col two">'.$date_code.$arr_meta['cats'].$title_line.'</div><div class="loop-col three">'.$meta_code.$sns_share_code.'</div>'.$js_code.'</article>';
...
function dp_show_post_list_for_archive_blog($args = array(), $arr_post_ele = array()) {
	...
	// Whole
	$loop_code .= 
'<article class="loop-article'.$col_class.$cat_class.$wow_article_css.'">'.$eyecatch_code.'<div class="loop-article-content">'.$title_line.$meta_code.$desc.$more_code.$sns_share_code.'</div>'.$js_code.'</article>';
...
function dp_show_post_list_for_archive_magazine1($args = array(), $arr_post_ele = array()) {
	...
	//Whole
	$loop_code .= 
'<article class="loop-article'.$col_class.$masonry_lines.$cat_class.$wow_article_css.'">'.$eyecatch_code.'<div class="loop-article-content">'.$title_line.'<div class="meta-wrapper">'.$meta_code.$desc.$more_code.$sns_share_code.'</div></div>'.$js_code.'</article>';
...
function dp_show_post_list_for_archive_magazine_mb($args = array(), $arr_post_ele = array()) {
	...
	if (!empty($embed_code)) {
		...
		$eyecatch_code = '<div class="loop-post-thumb '.$layout.'"><a href="'.$post_url.'" rel="bookmark" class="thumb-link r-wrap"><span>'.$thumbnail_code.'</span></a>'.$post_label.'</div>';
	}
	// Whole
	$loop_code .= 
'<article class="loop-article'.$col_class.$big_class.$masonry_lines.$cat_class.$wow_article_css.'"><div class="loop-col one">'.$eyecatch_code.'</div>'.$meta_code.$js_code.'</article>';
...
function dp_show_post_list_for_archive_slider($args = array(), $arr_post_ele = array()) {
	 ...
	 */
	$loop_code .= 
'<li class="loop-article'.$col_class.$no_cat_flag.$overlay_color.'"><div class="loop-post-thumb '.$layout.$overlay_color.'">'.$eyecatch_code.'<div class="loop-article-content">'.$arr_meta['cats'].'<div class="loop-table"><div class="loop-header">'.$arr_meta['date'].'<h1 class="entry-title loop-title '.$layout.$h1_fix_class.'"><a href="'.$post_url.'" rel="bookmark">'.$post_title.'</a></h1></div></div>'.$arr_meta['author'].$sns_share_code.$post_label.$arr_meta['views'].'</div></div></li>';
...
function dp_show_post_list_for_archive_news($args = array(), $arr_post_ele = array()) {
	 ...
	 */
	$loop_code .= 
'<article class="loop-article'.$wow_article_css.'">'.$arr_meta['date'].'<h1 class="entry-title loop-title '.$layout.'"><a href="'.$post_url.'" rel="bookmark">'.$post_title.'</a></h1>'.$arr_meta['author'].'</article>';
