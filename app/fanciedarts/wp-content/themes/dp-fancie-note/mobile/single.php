...
<div class="single-nav"><ul class="clearfix"><?php
			// Prev post
			if ($prev_post) {
				if ($post_type === 'post') {
					$nav_url = get_permalink($prev_post->ID);
					$nav_title = get_the_title($prev_post->ID);
					$arg_thumb = array('width' => 240, 'height' => 160, "size"=>"dp-widget-thumb", "if_img_tag"=> true, "post_id" => $prev_post->ID);
			...
			if ($next_post) {
				if ($post_type === 'post') {
					...
					$arg_thumb = array('width' => 240, 'height' => 160, "size"=>"dp-widget-thumb", "if_img_tag"=> true, "post_id" => $next_post->ID);
					$nav_img_code = DP_Post_Thumbnail::get_post_thumbnail($arg_thumb);
				}
