...
<?php 
						...
						foreach ($cats as $key => $cat) {
							...
							echo '<li>'.$cat->cat_name.' : <input type="text" name="cat_colors[]" class="dp-color-field" value="'.$this_cat_color.'" data-default-color="'.$options['base_link_color'].'" /><input type="hidden" name="cat_ids[]" value="'.$cat->term_id.'" /></li>';
						}
?>
									</ul>
								</div>
			...
			<dt class="dp_set_title1 icon-bookmark">ウィジェットエリア表示設定 :</dt>
				<dd class="mg20px-btm">
					<label>
						<input name="full_wide_container_widget_area_top" type="checkbox" value="check" <?php if($options['full_wide_container_widget_area_top']) echo "checked"; ?> />「コンテナエリア上部」スペースをフルワイド表示にする
					</label>

					<div class="cl-a slide-title icon-info-circled mg12px-top"><?php _e('Note...', 'DigiPress'); ?></div>
					<div class="slide-content">
					※「コンテナエリア上部」ウィジェットスペースの表示幅を、通常のコンテナエリアの固定幅から、表示幅に応じて可変するリキッドレイアウトにする場合にチェックします。
					</div>
				</dd>
				<dd class="mg20px-btm">
					<label>
						<input name="full_wide_container_widget_area_bottom" type="checkbox" value="check" <?php if($options['full_wide_container_widget_area_bottom']) echo "checked"; ?> /> 「コンテナエリア下部」スペースをフルワイド表示にする
					</label>

					<div class="cl-a slide-title icon-info-circled mg12px-top"><?php _e('Note...', 'DigiPress'); ?></div>
					<div class="slide-content">
					※「コンテナエリア下部」ウィジェットスペースの表示幅を、通常のコンテナエリアの固定幅から、表示幅に応じて可変するリキッドレイアウトにする場合にチェックします。
					</div>
				</dd>
		</dl>
<!-- 保存ボタン -->
