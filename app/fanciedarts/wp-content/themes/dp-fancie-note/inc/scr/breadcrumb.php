...
function dp_breadcrumb($echo = true,
	...
	if( !is_admin() ){
		...
		if (is_home() && is_paged()) {							// Home paged
			$tagTitleStart = '<span>';
			$str.='<li ' . $scope . ' itemtype="' . $itemType . '">' . $tagTitleStart . 'Page ' . get_query_var('paged') . $tagTitleEnd . '</li>';
			...
			if (is_category()) {
				...
				// Paged
				if (is_paged()) {
					$str.='<li ' . $scope . ' itemtype="' . $itemType . '">' . $tagTitleStart . $cat->name . ' ( ' . get_query_var('paged') . ' ) ' . $tagTitleEnd . '</li>';
					$blank_flag = false;
				...
				if (get_query_var('day') != 0){				// Archive of the day
					...
					// Paged
					if (is_paged()) {
						$str.='<li ' . $scope . ' itemtype="' . $itemType . '">' . $tagTitleStart . get_query_var('monthnum').' ( ' . get_query_var('paged') . ' ) ' . $tagTitleEnd . '</li>';
					} else {
					...
					// Paged
					if (is_paged()) {
						$str.='<li ' . $scope . ' itemtype="' . $itemType . '">' . $tagTitleStart . get_query_var('year') .' ( ' . get_query_var('paged') . ' ) ' . $tagTitleEnd . '</li>';
					} else {
				...
				// Paged
				if (is_paged()) {
					$str .='<li ' . $scope . ' itemtype="' . $itemType . '">' . $tagTitleStart  . get_the_author_meta('display_name', get_query_var('author')) . ' ( ' . get_query_var('paged') . ' ) ' . $tagTitleEnd . '</li>';
				} else {
				...
				// Paged
				if (is_paged()) {
					$str.='<li ' . $scope . ' itemtype="' . $itemType . '">' . $tagTitleStart . single_tag_title( '' , false ) . ' ( ' . get_query_var('paged') . ' ) ' . $tagTitleEnd . '</li>';
				} else {
				...
				// Paged
				if (is_paged()) {
					$str.='<li ' . $scope . ' itemtype="' . $itemType . '">' . $tagTitleStart . wp_title('', false) . ' ( ' . get_query_var('paged') . ' ) ' . $tagTitleEnd . '</li>';
				} else {
			...
			// Paged
			if (is_paged()) {
				$str.='<li ' . $scope . ' itemtype="' . $itemType . '">' . $tagTitleStart . $word . ' ( ' . get_query_var('paged') . ' ) ' . $tagTitleEnd . '</li>';
			} else {
