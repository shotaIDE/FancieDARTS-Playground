...
_EOD_;
	...
	// RGB
	$rgb_header_banner_txt_shadow = dp_hex_to_rgb($header_banner_text_shadow_color);
	$rgb_header_banner_txt = dp_hex_to_rgb($header_banner_font_color);
	$rgb_container_bg = dp_hex_to_rgb($container_bg_color);
	$rgb_base_font = dp_hex_to_rgb($base_font_color);
	$rgb_base_link = dp_hex_to_rgb($base_link_color);
	$rgb_hd_menu_bg = dp_hex_to_rgb($header_menu_bgcolor);
	$rgb_hd_menu_link = dp_hex_to_rgb($header_menu_link_color);
...
.meta-cat a{
	...
	foreach ($cat_ids as $key => $cat_id) {
		...
		if (!empty($cat_colors[$key])) {
			$rgb = dp_hex_to_rgb($cat_colors[$key]);
			$cat_colos_css .= 
