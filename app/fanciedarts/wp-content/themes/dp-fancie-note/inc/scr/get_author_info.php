...
function dp_get_author_info($args = array(
				...
				'is_microdata' 	=> true,
				'avatar_size' 	=> 300) ) {
	global $post, $options, $IS_MOBILE_DP, $IS_AMP_DP;
	...
	if ($IS_AMP_DP) {
		$avatar_img = preg_replace('/<img ([^>]+?)\/?>/i', '<amp-img width="130" height="130" $1></amp-img>', $avatar_img);
	}

	// Profile data
	$author_name 	= '<span'.$mi_name.' class="author_name">'.get_the_author_meta('display_name', $user_id).'</span>';
	$profile_img = '<a href="'.get_author_posts_url($user_id).'" rel="author" title="'.__('Show articles of this user.', 'DigiPress').'" class="author_img">'.$avatar_img.'</a>';
	...
	$author_bg_img 	= empty($author_bg_img) && !$IS_AMP_DP ? '' : ' style="background:url('.$author_bg_img.') center center;background-size:cover;"';
...
function dp_get_author_posts() {
	...
	if ($articles) {
		...
		foreach ( $articles as $post ) : setup_postdata( $post );
			...
			if (!$IS_AMP_DP) {
				$thumb_code = '<div class="widget-post-thumb"><a '.$anchor_code.' class="'.$thumb_link_class.'"><figure class="post-thumb" style="background-image:url(\''.$thumb_code.'\');"></figure></a></div>';
			} else {
				$thumb_code = '<a '.$anchor_code.' class="'.$thumb_link_class.'"><figure class="loop-post-thumb">'.$thumb_code.'</figure></a>';
			}

			$articles_code .= $thumb_code;
			$articles_code .= '<div class="excerpt_div has_thumb">';
			// Date
			...
			// Title
			$articles_code .= '<h4 class="entry-title"><a '.$anchor_code.' class="'.$item_link_class.'">'.$title.'</a></h4>';
			$articles_code .= '</div>'; // end of .excerpt_div
			// Close list
			$articles_code .= '</li>';
