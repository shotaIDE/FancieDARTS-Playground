<div class="r_block"><?php
// **********************************
// SNS icon and search form
// **********************************
if ($options['global_menu_right_content'] !== 'none') :?>
<div id="hd_misc_div"<?php echo ' class="hd_misc_div'.$wow_title_css.'"'.$attr_delay_2; ?>><?php
	if ($options['global_menu_right_content'] === 'sns') {
		...
		if ($options['show_global_menu_rss']) {
			$sns_code .= $options['rss_to_feedly'] ? '<li class="feedly"><a href="https://feedly.com/i/subscription/feed/'.urlencode(get_feed_link()).'" target="_blank" title="Follow on feedly"><span class="r-wrap"><i class="icon-feedly"></i></span></a></li>' : '<li class="rss"><a href="'. get_feed_link() .'" title="Subscribe Feed" target="_blank"><span class="r-wrap"><i class="icon-rss"></i></span></a></li>';
		}
		// Show
		if (!empty($sns_code)) {
...
<div id="hd_searchform"<?php if ($IS_MOBILE_DP) echo ' class="mb"'; ?>><?php
			if ($options['show_floating_gcs']) {
				// Google Custom Search?>
<div id="dp_hd_gcs"><gcse:searchbox-only></gcse:searchbox-only></div><?php
			} else {
				// Default search form?>
<span class="r-wrap"><i id="hd_search_btn" class="icon-search"></i></span><?php
			}?>
</div><?php
...
function show_wp_global_menu_list() { 
	// Global scope
	global $IS_MOBILE_DP;?>
<ul id="global_menu_ul"><li class="menu-item"><a href="<?php echo home_url(); ?>" title="HOME" class="icon-home menu-link"><?php _e('HOME','DigiPress'); ?></a></li><li class="menu-item"><a href="<?php echo get_feed_link(); ?>" target="_blank" title="feed" class="icon-rss menu-link">RSS</a></li></ul><?php
} //End Function
