...
if (!is_404() && is_active_sidebar('widget-container-bottom')) {
	...
	if (isset($options_visual['full_wide_container_widget_area_bottom']) && !empty($options_visual['full_wide_container_widget_area_bottom'])) {
		$append_widget_container_bottom_class = ' liquid';
	}
	// Display
	echo '<div id="widget-container-bottom" class="widget-container bottom clearfix'.$append_widget_container_bottom_class.'">'.$widget_container_bottom_content.'</div>';
}	// is_active_sidebar('widget-container-bottom')
// **********************************
// Footer
// **********************************?>
<footer id="footer" class="clearfix<?php echo $show_header_class; ?>">
<div class="ft-container"><?php
// **********************************
...
<i id="gotop" class="pc icon-up-open"></i><?php
// **********************************
// Hidden Search form (Header trigger)
// **********************************
if ($options['show_global_menu_search'] && !$options['show_floating_gcs']) {?>
<div id="hidden-search-wrapper"><form method="get" id="hidden-searchform" action="<?php echo esc_url(home_url('/')); ?>"><input type="search" id="hidden-searchtext" class="hidden-searchtext" name="s" placeholder="<?php esc_attr_e( 'Type to search...', 'DigiPress' ); ?>" required /></form><i class="hidden-close-btn"></i></div><?php
}
// **********************************
// pace.js options (*Need before main script)
// **********************************?>
<script>paceOptions={restartOnRequestAfter:false};</script><?php
// **********************************
// WordPress Footer
// **********************************
wp_footer();
